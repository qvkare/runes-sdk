...
export class RunesOrderService {
  ...
  private createOrderTransaction(token: string, amount: string, price: string, sender: string, recipient: string): Transaction {
  ...
  async createOrder(params: { token: string, amount: string, price: string }): Promise<string> {
    try {
      const tx: Transaction = this.createOrderTransaction(params.token, params.amount, params.price, '', '');
