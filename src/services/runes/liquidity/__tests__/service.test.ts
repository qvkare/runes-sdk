...
describe('RunesLiquidityService', () => {
  ...
  describe('createPool', () => {
    ...
    it('should handle error when creating pool', async () => {
      ...
      await expect(service.createPool(runeId, initialLiquidity)).rejects.toThrow('Failed to create rune pool');
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to create rune pool:', expect.any(Error));
    });
  });
  ...
  describe('addLiquidity', () => {
    ...
    it('should handle error when adding liquidity', async () => {
      ...
      await expect(service.addLiquidity(runeId, amount)).rejects.toThrow('Failed to add rune liquidity');
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to add rune liquidity:', expect.any(Error));
    });
  });
