...
describe('RunesSecurityService', () => {
  ...
  describe('validateRuneAccess', () => {
    it('should validate rune access successfully', async () => {
      ...
      expect(mockRpcClient.call).toHaveBeenCalledWith('validateruneaccess', [runeId, address, action]);
    });
    ...
    it('should handle validation error', async () => {
      ...
      await expect(service.validateRuneAccess(runeId, address, action)).rejects.toThrow('Failed to validate rune access');
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to validate rune access:', expect.any(Error));
    });
  });
  ...
  describe('getRunePermissions', () => {
    ...
    it('should handle error when getting permissions', async () => {
      ...
      await expect(service.getRunePermissions(runeId)).rejects.toThrow('Failed to get rune permissions');
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to get rune permissions:', expect.any(Error));
    });
  });
  ...
  describe('updateRunePermissions', () => {
    it('should update rune permissions successfully', async () => {
      ...
      expect(result).toBe(true);
      expect(mockRpcClient.call).toHaveBeenCalledWith('updaterunepermissions', [runeId, permissions]);
    });
    ...
    it('should handle error when updating permissions', async () => {
      ...
      await expect(service.updateRunePermissions(runeId, permissions)).rejects.toThrow('Failed to update rune permissions');
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to update rune permissions:', expect.any(Error));
    });
  });
});
